...
- **`Esc`는 넓힌 것부터 되돌린다**: 관리 컬럼이 펼쳐져 있으면 접고(팝업은 열린 채), 접혀 있으면 팝업을 닫는다. 검색 필드에 포커스가 있어도 같다.
  - Esc 수신은 `.onExitCommand`가 아니라 **팝업이 떠 있는 동안만 사는 키다운 로컬 모니터**다. `.onExitCommand`는 포커스가 있는 뷰에만 오므로 관리 컬럼이 접혀 있으면(팝업 안에 포커스 대상이 없다) 이벤트가 아예 안 온다 — 실측에서 컬럼 펼친 Esc만 먹고 접힌 Esc는 무반응이었다. 모니터는 `PopupView`의 `onAppear`에서 걸고 `onDisappear`에서 뗀다(팝업 여닫을 때마다 새로 걸리므로 안 떼면 누적된다).
