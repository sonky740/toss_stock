...
- **상태별 표시**:
  ...
  - 관심종목 0건 → `관심종목 없음` (회색).
- **행 클릭(종목 페이지 열기)**: §3.2와 동일하다(한국 딥링크 / 미국 홈+툴팁, `currency=="USD"` 판별, 키보드는 §3.8). 단 **조회실패 행**은 종목명·통화 조회가 안 돼 `currency`가 KRW로 강제 채워지므로, 이 경우에만 **종목코드 첫 글자**로 판별한다(숫자면 한국, 아니면 미국).
...
- **펼치면 검색 필드에 포커스가 간다.** 진입 행을 누르는 의도가 대개 검색이라 바로 타이핑할 수 있어야 한다. 접었다 펴면 `SearchSection`이 다시 그려지므로 매번 포커스가 잡힌다.
  - `onAppear`에서 `@FocusState`를 **곧바로** 세우면 안 붙는다 — 컬럼이 뷰 트리에 드는 순간 패널이 360→681로 리사이즈되며 first responder 가 리셋된다. 50ms 미뤄 세운다. 리사이즈가 없는 별칭 인라인 편집(아래)은 즉시 세워도 붙는 것이 이 원인의 대조군이다.
...
- **검색**: 관심종목을 추가하는 **유일한 경로**다. 종목코드를 몰라도 이름으로 찾아 등록한다. 코드를 직접 입력하는 별도 줄은 없다 — 검색이 심볼도 매칭하므로 `005930`·`AAPL`·`0190C0`을 그대로 입력하면 같은 결과가 나온다.
  ...
  - **키보드**: 검색 필드가 포커스된 채 `↓`/`↑`로 결과 행을 옮기고 `Enter`로 선택 행을 등록한다(`+` 버튼과 같은 동작, 이미 등록된 행이면 무동작).
    - **선택이 없을 때의 `Enter`는 아무것도 하지 않는다** — 타이핑 끝에 무심코 누른 Enter 로 엉뚱한 종목이 등록되면 안 된다. 맨 위에서 `↑`를 한 번 더 누르면 선택이 풀린다.
    - 결과가 없으면 `↑`/`↓`를 가로채지 않는다(`.ignored`) — 필드의 캐럿 이동을 뺏지 않는다. 질의가 바뀌면 결과가 통째로 갈리므로 선택도 초기화한다.
    - 선택 행이 뷰 밖이면 `ScrollViewReader`로 끌어온다. 선택 표시는 드롭 타깃과 같은 인디고 틴트(`Palette.dropHi`)라 hover(흰 5%)와 겹쳐도 구분된다.
...
- **`Esc`는 넓힌 것부터 되돌린다**: 관리 컬럼이 펼쳐져 있으면 접고(팝업은 열린 채), 접혀 있으면 팝업을 닫는다. 검색 필드에 포커스가 있어도 같다.
  - Esc 수신은 `.onExitCommand`가 아니라 **팝업이 떠 있는 동안만 사는 키다운 로컬 모니터**다(본체 컬럼 키보드 이동 §3.8과 같은 모니터). `.onExitCommand`는 포커스가 있는 뷰에만 오므로 관리 컬럼이 접혀 있으면(팝업 안에 포커스 대상이 없다) 이벤트가 아예 안 온다 — 실측에서 컬럼 펼친 Esc만 먹고 접힌 Esc는 무반응이었다. 모니터는 `PopupView`의 `onAppear`에서 걸고 `onDisappear`에서 뗀다(팝업 여닫을 때마다 새로 걸리므로 안 떼면 누적된다).
  - 팝업 닫기는 **상태아이템 버튼을 대신 눌러**(`NSStatusBarButton.performClick`) SwiftUI 자신의 토글 경로를 태운다. 패널 `close()`는 창만 없애고 `MenuBarExtra`의 표시 상태를 안 풀어, 다음 상태아이템 클릭이 "닫기"로 소비돼 **두 번 눌러야** 다시 열렸다(실측). 버튼은 SwiftUI 가 소유해 참조를 안 주므로 자기 앱 창에서 찾고, 못 찾을 때만 `close()`로 떨어진다.
...
팝업이 떠 있는 동안 마우스 없이 본체 컬럼을 오간다. 순서는 화면 순서 그대로 **진입 행(§3.4) → 보유종목(§3.2) → 관심종목(§3.3) → 하단 버튼(§3.5)** 이다.

- **이동**: `↓`/`Tab` 다음, `↑`/`Shift-Tab` 이전. 끝에서 반대편으로 순환한다. 링이 없는 상태에서는 **방향과 무관하게 첫 항목**(진입 행)이다 — `↑`를 마지막(`종료`)으로 보내면 갓 연 팝업에서 두 키에 앱이 꺼진다.
- **실행**: `Enter`(키패드 Enter 포함). 진입 행은 관리 컬럼 여닫기, 종목 행은 종목 페이지 열기(마우스 클릭과 같은 동작·같은 KR/US 판별), 하단은 각 버튼 동작이다. 링이 없으면 아무것도 하지 않고 이벤트를 흘린다.
- **표시는 인디고 링(stroke)이지 배경 틴트가 아니다.** 틴트는 드래그 드롭 타깃 하이라이트(`Palette.dropHi`)와 같은 자리·같은 색이라 재배치 중 둘을 구분할 수 없다.
- **링은 인덱스가 아니라 종목코드로 가리킨다.** 10초 폴링으로 목록이 갈려도 링이 다른 종목으로 밀리지 않는다. 링이 가리키던 종목이 사라졌으면 다음 이동에서 첫 항목으로 돌아간다. 보유·관심에 같은 코드가 있어도 서로 다른 지점이다(스크롤 앵커도 이 값을 쓴다 — 코드 문자열만 쓰면 두 섹션이 겹친다).
- 링이 뷰 밖이면 `ScrollViewReader`로 끌어온다. 진입 행·하단 버튼은 `ScrollView` 밖이라 대상이 아니다.
- **수신은 Esc와 같은 키다운 로컬 모니터**(§3.4)다. `.onKeyPress`/`.focusable()`은 쓰지 않는다 — 전자는 포커스가 있는 뷰에만 오는데 이 팝업은 포커스 대상 없이 떠 있는 게 기본이고, 후자의 Tab 링은 시스템 설정 *키보드 탐색*(기본 꺼짐)에 좌우돼 켠 사람에게만 동작한다.
- **텍스트 입력 중에는 Esc 말고 아무 키도 가로채지 않는다.** first responder 가 `NSTextView`/`NSTextField` 면 그대로 흘린다 — 검색 필드의 `↑`/`↓`/`Enter`(§3.4)와 한글 조합을 뺏으면 안 된다. 그래서 **관리 컬럼이 펼쳐져 있는 동안은 본체 링이 키보드로 닿지 않는다**(검색 필드가 포커스를 쥔다). `Esc`로 접거나 클릭으로 포커스를 뺀 뒤 쓴다. 관리 컬럼을 펼치는 순간 링은 지운다 — "지금 여기"가 둘이면 안 된다.
- 자격증명 미설정 화면(§3.6)에서는 링이 없다.
