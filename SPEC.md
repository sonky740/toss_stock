...
### 3.4 종목 검색·관리 (드롭다운 옆 컬럼)

검색·추가·수정·삭제는 드롭다운 **최상단 고정 행**(`종목 검색·관리 · N개 등록됨`)을 눌러 여닫는다(osascript 다이얼로그 아님). 별도 창이 아니라 **같은 패널이 가로로 넓어져** 옆에 관리 컬럼이 붙는다. 다시 누르면 접힌다.

- **진입 행은 ScrollView 밖**에 둔다. 안에 두면 보유·관심 목록이 길 때 스크롤에 밀려나 "항상 맨 위"가 깨진다.
- **붙는 쪽은 화면 남는 폭이 정한다.** **오른쪽이 기본**이고, 패널 오른쪽에 컬럼 폭(320)이 안 남으면 왼쪽에 붙인다. 양쪽 다 부족하면 오른쪽으로 둔다(`MenuBarExtra` 패널이 화면 안으로 알아서 재배치한다). 방향은 **펼칠 때만** 정한다 — 접는 중에 바뀌면 컬럼이 반대편으로 튀었다가 사라진다.
  - 기준 화면은 **패널이 놓인 화면**(`NSScreen.screens.first { $0.frame.intersects(anchor) }`)이고 `visibleFrame`을 쓴다. `NSScreen.main` 폴백은 쓰지 않는다 — key window 가 없으면 아무 화면이나 주고, 실측(별도 창 시안)에서 창이 다른 화면(x = -1160)으로 갔다. 앵커나 그 화면을 못 찾으면 기본값 오른쪽.
  - 패널의 화면 위치는 SwiftUI 가 씬의 `NSWindow`를 노출하지 않아 `PopupPanelGrabber`(`viewDidMoveToWindow`, `ScrollViewGrabber`와 같은 수법)로 잡는다. 프레임은 캐시하지 않고 **펼칠 때 다시 읽는다** — 회전 타이틀 길이에 따라 상태아이템 폭이 바뀌어 패널이 가로로 움직인다. 앵커는 약참조이고, 팝업이 닫히며 오는 `window == nil` 호출은 무시한다(덮으면 앵커를 잃는다).
  - 좌우 판정은 순수 함수 `PopupAnchor.preferredSide(anchor:visible:paneWidth:)`로 떼어 둔다. 메뉴바 패널이 합성 클릭에 반응하지 않아 이 계산만은 따로 검사할 수 있어야 한다.
- **두 컬럼 높이는 같다.** 관리 컬럼의 `ScrollView`는 보유·관심 컬럼이 쓰는 실측 높이(`min(max(contentHeight, 60), 520)`, §4.2)를 **최소 높이**로 받고 남는 높이를 먹어 아래를 맞춘다(`minHeight:maxHeight: .infinity`). 자기 높이를 따로 재면 self-sizing 패널에서 0으로 붕괴하고, `maxHeight`만 주면 HStack 높이 계산에 따라 납작해질 수 있다.
  - `HStack` 정렬은 **`.top`** 이어야 한다. 기본 `.center` 면 본체보다 짧은 관리 컬럼이 세로 가운데로 밀려 위아래에 여백이 생긴다(진입 행 + 하단바 높이만큼, 실측 위쪽 ~23pt).
  - 진입 행과 컬럼 헤더는 **높이를 공유한다**(`popupHeaderHeight` = 40 = iconBadge 22 + 패딩 9×2). 같은 패딩만 주면 진입 행은 배지가, 컬럼 헤더는 텍스트가 높이를 정해 어긋나고, 두 컬럼의 첫 구분선이 다른 줄에 그려진다.
- **컬럼 폭 320**, 본체 360. 펼치면 패널 전체가 681(=360+1+320)이 된다.
- **문구는 진입 행·컬럼 헤더 모두 `종목 검색·관리`** 다(진입 행엔 `N개 등록됨` 병기 — 관심종목 개수). 이 면은 관심종목 편집만이 아니라 상장 종목 전체(15,176) 검색도 하므로 관심종목만 가리키는 이름은 좁다.
- **접었다 펴도 검색 상태가 남는다**: `query`·검색 결과는 `ManagePane`의 `@State`와 `model.searchResults`에 있고, 컬럼이 뷰 트리에서 빠지면 `query`는 초기화되지만 `model.searchResults`는 남는다. `SearchSection`은 질의가 비어도 결과가 있으면 그리므로, 빈 검색창 아래 이전 결과가 보인다.
- **컬럼 레이아웃**: 헤더 → 구분선 → (검색 필드 → 검색 결과 → `등록된 종목 N개` → 등록 행). 마지막 라벨은 검색 결과와 등록 목록의 경계를 만든다.
...
- **검색**: 관심종목을 추가하는 **유일한 경로**다. 종목코드를 몰라도 이름으로 찾아 등록한다. 코드를 직접 입력하는 별도 줄은 없다 — 검색이 심볼도 매칭하므로 `005930`·`AAPL`·`0190C0`을 그대로 입력하면 같은 결과가 나온다.
  - **유니버스**: 토스 Open API에 검색 엔드포인트가 없어 `/stocks/all`로 마켓 7개(`KOSPI·KOSDAQ·NYSE·NASDAQ·AMEX·KR_ETC·US_ETC`) 전체 목록을 받아 로컬에서 찾는다. `STOCK_ALL`이 초당 1회라 호출 사이 1.2초를 두므로 콜드 수집에 약 8초가 들고, 그동안 `종목 목록 준비 중… (n/7)`을 표시한다. 수집은 **관리 컬럼을 처음 펼칠 때** 시작한다 — 검색을 안 쓰는 세션에서는 호출이 나가지 않는다. 이후는 `universe.json` 캐시(§2.3)로 즉시 검색된다.
  ...
  - **추가**: 행 우측 `+` 버튼 하나다. **별칭은 추가 시점에 받지 않는다** — 등록하면 종목명으로 표시되고, 별칭이 필요하면 아래 등록 목록에서 표시명을 클릭해 지정한다. 검색 행에 입력칸을 두면 질의가 바뀔 때마다 입력이 엉뚱한 종목에 붙지 않게 비워야 하고, 좁은 컬럼에서 행 폭도 잡아먹는다. 이미 등록된 종목은 버튼 대신 `✓` 배지를 보여준다(행 클릭은 그대로 종목 페이지를 연다).
...
│   ├── Popup/                     화면 레이어
│   │   ├── PopupView.swift        드롭다운(관리진입/보유/관심/하단/설정안내) — 다크 'Color pill' 디자인
│   │   ├── ManageView.swift       관심종목 관리 컬럼(ManagePane) + PopupAnchor 좌/우 판정(§3.4)
│   │   ├── SearchSection.swift    관리 컬럼 안의 종목 검색 UI(§3.4)
