...
struct TossService: Sendable {
    ...
    func watchRows(_ symbols: [WatchSymbol]) async throws -> [WatchRow] {
        ...
        for sym in symbols {
            guard let price = priceBy[sym.code], let last = price.lastPrice else {
                rows.append(WatchRow(id: sym.code, rowName: sym.code, titleName: sym.code,
    ...
    func authStatus() async throws -> AuthStatus {
        ...
        return AuthStatus(accountNo: first.accountNo, accountSeq: first.accountSeq,
    ...
    private func prevClose(_ code: String) async -> Double? {
        guard let page = try? await api.get("/api/v1/candles?symbol=\(code)&interval=1d&count=2",
                                            as: CandlePageResponse.self),
              page.candles.count >= 2 else { return nil }   // 신규상장 등 count<2 인덱스 가드
...
enum Dump {
    ...
    static func run() async {
        ...
        do {
            ...
            print("  accountNo=\(mask(a.accountNo)) seq=\(a.accountSeq) expiry=\(a.expiresAt.map(String.init(describing:)) ?? "nil")")
        ...
        do {
            ...
            for r in rows {
                print("  [\(tag(r.currency))] \(r.id) \(r.name)  last=\(r.lastPrice)  pct=\(String(format: "%+.2f", r.ratePercent))%  pnl=\(r.pnlAmount)  \(r.direction)")
            }
        } catch { print("  FAILED: \(error)") }
        ...
        do {
            ...
            print("  count=\(rows.count)")
            for r in rows {
                print("  [\(tag(r.currency))] \(r.id) row=\(r.rowName) title=\(r.titleName) last=\(r.lastPrice) change=\(r.change) resolved=\(r.resolvedName ?? "-")")
            }
        } catch { print("  FAILED: \(error)") }
    }

    private static func tag(_ c: Currency) -> String {
        switch c { case .krw: "KRW"; case .usd: "USD"; case .unknown(let s): "?\(s)" }
    }
